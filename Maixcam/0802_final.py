...
while not app.need_exit():
    ...
    if MODE == -1 and mode6flag == 0:
        ...
        if pressed:  # 只有在触摸按下时才处理
            handle_touch(x, y)
    #######################################################模式0休眠模式#################################################
    elif MODE == 0 and mode6flag == 0:
        print("休眠")
    #######################################################模式1基础模式##################################################
    elif MODE == 1 and mode6flag == 0:
        # 预处理
        edges, gray_eq = preprocess_image(img_cv)
    ...
    #############################################模式2发挥题###########################################
    elif (MODE == 2 or MODE == 4 ) and mode6flag == 0:
        ...
        ############################################################################################################
    #################################模式4作弊模式#######################################################
    elif (MODE == 3 or MODE == 5 ) and mode6flag == 0:
    #######代表已经接收到了数据并且接收到了命令##################################################################
        if Number != -1:
            gray = cv2.cvtColor(img_cv, cv2.COLOR_BGR2GRAY)
            blur = cv2.GaussianBlur(gray, (5, 5), 0)
            ...
            if warped_inner is not None and warped_inner.size > 0:
                ...
                # 【核心修改1：按矩形大小排序】
                if len(rectCoords) > 0 and len(rectCoords) <= Number:
                    # 定义计算矩形尺寸的函数（外接矩形面积）
                    def get_rect_size(quad_array):
                        ...
                        return w * h  # 用宽高乘积表示尺寸
                    
                    # 按尺寸升序排序（从小到大），如需降序添加 reverse=True
                    rectCoords.sort(key=get_rect_size)
                    pixelWidth = (rectCoords[Number - 1][0][0] - rectCoords[Number - 1][1][0]) ** 2 + (rectCoords[Number - 1][0][1] - rectCoords[Number - 1][1][1]) ** 2
                    sqrtWidth  = math.sqrt(pixelWidth)
                    
                    real = sqrtWidth / TARGET_HEIGHT * KNOWN_HEIGHT * 10
                    distance   = distance_cm * 10
                    print(int(distance), int(real))
    ...
    ##################################模式5作弊模式######################################################
        ...
        if warped_inner is not None and warped_inner.size > 0:
            ...
            binary = cv2.bitwise_not(binary_image)
            newImage = get_only_largest_white_component_max(binary)
            
            # 安全检查：确保newImage不是None
            if newImage is not None:
                newImage = fill_holes_in_white(newImage)
                contour = get_innermost_contours(newImage)
                rectCoords = detector.DEBUG_FindIOURects(contour, frame = warped_inner)
            else:
                print("错误：get_only_largest_white_component_max返回None，跳过后续处理")
                rectCoords = []
